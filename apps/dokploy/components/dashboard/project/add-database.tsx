...
const dockerImageDefaultPlaceholder: Record<DbType, string> = {
  ...
  redis: "redis:8",
  sqlserver: "mcr.microsoft.com/mssql/server:2025-latest",
};
  ...
  .discriminatedUnion("type", [
    ...
    z
      .object({
        ...
        databaseUser: z.string().default("sa"),
        databaseName: z.string().default("sqlserver"),
      })
      .merge(baseDatabaseSchema),
  ])
...
export const AddDatabase = ({ environmentId, projectName }: Props) => {
  ...
  const sqlServerMutation = api.sqlserver.create.useMutation();
  ...
  const onSubmit = async (data: AddDatabase) => {
    ...
    if (data.type === "libsql") {
      ...
      promise = sqlServerMutation.mutateAsync({
        ...commonParams,
        databasePassword: data.databasePassword,
        serverId: data.serverId === "dokploy" ? null : data.serverId,
      });
