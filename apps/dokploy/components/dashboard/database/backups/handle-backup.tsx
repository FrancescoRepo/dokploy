...
const Schema = z
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.backupType === "compose" && data.databaseType) {
      ...
      } else if (data.databaseType === "sqlserver") {
        if (!data.metadata?.sqlserver?.databaseRootPassword) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: "Root password is required for SQLServer",
            path: ["metadata", "sqlserver", "databaseRootPassword"],
          });
        }
      }
    }
